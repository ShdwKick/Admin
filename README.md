...
управлять ролью админа. Почти ничего не хранит сам — при каждом обращении
дёргает `/internal/*` настроенных сервисов и отдаёт фронту; единственное
исключение — health-check по расписанию (см. ниже), которому нужно помнить
флаг «известная проблема» между рестартами. Дизайн — калий, сиреневое пламя,
см. `BurningHouse/Design/services.md` и `palette.md`.
...
| `HEALTH_CHECK_INTERVAL_MS` | нет (по умолчанию `3600000` = 1 час) | как часто проверять доступность сервисов |
| `DATA_DIR` | нет (по умолчанию `./data`) | где лежит `health-state.json` — единственное, что Admin хранит на диске |
